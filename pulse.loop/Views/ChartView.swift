...
struct ChartView<CharacteristicType: GeneralCharacteristicProtocol<AggregatedData>>: View {
    ...
    private func updatePathCache(dataPoints: WritableKeyPath<Self, State<[DataPoint]>>) {
        ...
        // Add new point.        
        result.append(self.getCoordinates(for: self[keyPath: dataPoints].wrappedValue.last ?? (0, CACurrentMediaTime()), in: self.size))
            ...
            Canvas(rendersAsynchronously: false) { context, fullSize in
                ...
                for (keyPath, color) in zip(pathsKeyPaths, colors) {
                    if let path = self.paths[keyPath] {
                        context.stroke(path.offsetBy(dx: leftPadding, dy: 0), with: .color(color), style: StrokeStyle(lineWidth: 2))
                    }
                }
                
                // Areas.
                let areasKeyPaths: [(WritableKeyPath<Self, State<[DataPoint]>>, WritableKeyPath<Self, State<[DataPoint]>>)] = [
