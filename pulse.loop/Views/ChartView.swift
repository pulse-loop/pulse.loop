...
struct ChartView<T, CharacteristicType: GeneralCharacteristicProtocol<T>>: View where T: PlottableData {
    ...
    private func update() {
        for channel in characteristic.value.getChannels() {
            ...
            if let min = channels[channel.specifier]?.min(by: { a, b in a.value < b.value }),
               let max = channels[channel.specifier]?.max(by: { a, b in a.value < b.value }),
               max.value - min.value > 0 {
                self.range = (min: min.value, max: max.value)
            }
        }
    }
